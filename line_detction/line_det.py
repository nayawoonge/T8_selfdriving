...
def wrapping(image):
    ...
    # 좌상, 우상, 좌하, 우하
    source = np.float32([[600,380],[740,380],
                         [370,500],[935,500]])
...
def slide_window_search(binary_warped, left_current, right_current):
    ...
    # lefty와 leftx의 크기가 0인 경우에 예외 처리
...
while True:
    ...
    if key == 27 :  #esc
        ...
        print("Capture")
        cv2.imwrite(str(now) + ".png", result)
    elif key == 114 :  # r 영상 녹화
        print("Video Record")
        record = True
        video = cv2.VideoWriter(str(now) + ".avi", cv2.VideoWriter_fourcc(*'XVID'), 20.0, frame_size)
