...
def detect_and_draw_lane(image):
    # Apply color filtering
    filtered_image = color_filter(image)

    # Apply region of interest mask
    masked_image = roi(filtered_image)

    # Apply perspective transformation
    wrapped_image, Minv = wrapping(masked_image)

    # Detect lane lines using sliding window search
    histogram = plothistogram(wrapped_image)
    leftbase, rightbase = histogram
    draw_info = slide_window_search(wrapped_image, leftbase, rightbase)

    # Draw the lane lines on the original image
    pts_mean, result = draw_lane_lines(image, wrapped_image, Minv, draw_info)

    return result


cap = cv2.VideoCapture('C:/Users/LISA/Desktop/test4.mov')
...
while True:
    ...
    result = detect_and_draw_lane(frame)
